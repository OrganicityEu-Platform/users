...
var ScenariosCanvas = React.createClass({
  ...
  render: function() {
    return (
      <div className="oc-canvas-pack" ref="oc-canvas-pack">

          {
            this.state.builtItems.map(
              (item) =>
              <div className={"oc-canvas-item " + item.sector + "_colour " + item.size}>
                <img className="oc-canvas-item-icon" src={ui.asset('static/img/'.concat(item.sector.concat('_icon.svg')))}/>
                <span className="oc-canvas-item-text">{item.sector.toUpperCase()}</span>
